...
const loginAdminUser = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ msg: "Login realizado com sucesso", token, userId: user._id, isAdmin: true });
  } catch (error) {
...
const getAdminUser = async (req, res) => {
  ...
  if(!userId) {
    res.status(401).send({ msg: " Inserir userId" });
    ...
    try {
      ...
      return res.status(500).json({ msg: "Erro interno ao encontrar usuario", error });
    }
}
}
